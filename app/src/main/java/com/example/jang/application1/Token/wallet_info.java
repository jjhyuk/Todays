...
public class wallet_info extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.d("Key Value : ",Key);
        walletAddr = sharedPreferences.getString("walletAddr","에러마스터키");
        Log.d("walletAddr Value : ",walletAddr);
        ...
        getEtherBtn = findViewById(R.id.getEther);
        getEtherBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                getEther ge = new getEther();
                ge.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
            }
        });

        getTokenBtn = findViewById(R.id.getToken);
    ...
    private SeekBar.OnSeekBarChangeListener seekBarChangeListenerGL = new SeekBar.OnSeekBarChangeListener() {
        ...
        public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
            GetGasLimit(String.valueOf(seekBar.getProgress()*1000+100000));
        }
    ...
    public void GetFee(){
        GasPrice = Convert.toWei("110",Convert.Unit.GWEI).toBigInteger();
        GasLimit = BigInteger.valueOf(Integer.valueOf(String.valueOf("100000")));
    ...
    ///////////////////////// Sending Ether //////////////////////
    public class getEther  extends AsyncTask<Void, Integer, org.json.simple.JSONObject> {
        ...
        protected org.json.simple.JSONObject doInBackground(Void... param) {
            ...
            File KeyDir = new File(DataDir.getAbsolutePath());
            File[] listfiles = KeyDir.listFiles();
            File file = new File(KeyDir.getAbsolutePath()+"/"+Master_Name);
            ...
            try {
                 ...
                 * Convert ammount ether to BigInteger
                 */
                BigInteger value = Convert.toWei(String.valueOf(0.05), Convert.Unit.ETHER).toBigInteger();

                /**
                 * Транзакция
                 * Transaction
                 */
                RawTransaction rawTransaction  = RawTransaction.createEtherTransaction(nonce, GasPrice, GasLimit, walletAddr, value);
                byte[] signedMessage = TransactionEncoder.signMessage(rawTransaction, credentials);
                String hexValue = "0x"+ Hex.toHexString(signedMessage);
                EthSendTransaction ethSendTransaction = web3.ethSendRawTransaction(hexValue.toString()).sendAsync().get();
                 ...
                 * Get Transaction Error and Hash
                 */
                System.out.println("Error1: "+ ethSendTransaction.getError());
                System.out.println("Transaction: " + ethSendTransaction.getTransactionHash());
    ...
    ///////////////////////// Sending Tokens /////////////////////
    public class getToken extends AsyncTask<Void, Integer, org.json.simple.JSONObject> {
        ...
        protected org.json.simple.JSONObject doInBackground(Void... param) {
            ...
            try {
                 ...
                 * Convert the amount of tokens to BigInteger and send to the specified address
                 */
                BigInteger sendvalue = BigInteger.valueOf(Long.parseLong(String.valueOf(500)));
                Log.d("Token Num", String.valueOf(Convert.toWei(String.valueOf(sendvalue),Convert.Unit.ETHER).toBigInteger()));
                status = token.transfer(String.valueOf(walletAddr), Convert.toWei(String.valueOf(sendvalue),Convert.Unit.ETHER).toBigInteger()).send().getTransactionHash();
        ...
        protected void onPostExecute(org.json.simple.JSONObject result) {
            ...
            if (result != null) {
                System.out.println("거래주소: " +result.get("status").toString());
                Toast.makeText(getApplicationContext(),result.get("status").toString(),Toast.LENGTH_SHORT);
